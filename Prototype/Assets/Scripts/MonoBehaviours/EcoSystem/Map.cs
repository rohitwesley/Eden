...
public class Map {
    public readonly List<AgentInfo>[, ] map;
    ...
    public Map (int size, int regionSize) {
        ...
        map = new List<AgentInfo>[numRegions, numRegions];
        ...
        for (int y = 0; y < numRegions; y++) {
            for (int x = 0; x < numRegions; x++) {
                ...
                centres[x, y] = centre;
                map[x, y] = new List<AgentInfo> ();
            }
        }
    }

    public AgentInfo ClosestEntity (Coord origin, float viewDistance, bool[,] walkable) {
        List<RegionInfo> visibleRegions = GetRegionsInView (origin, viewDistance);
        AgentInfo closestEntity = null;
        ...
        for (int i = 0; i < visibleRegions.Count; i++) {
            ...
            for (int j = 0; j < map[regionCoord.x, regionCoord.y].Count; j++) {
                AgentInfo entity = map[regionCoord.x, regionCoord.y][j];
                float sqrDst = Coord.SqrDistance (entity.coord, origin);
                if (sqrDst < closestSqrDst) {
                    if (EnvironmentUtility.TileIsVisibile(origin.x, origin.y, entity.coord.x, entity.coord.y, walkable)) {
    ...
    public void Add (AgentInfo e, Coord coord) {
    ...
    public void Remove (AgentInfo e, Coord coord) {
        int regionX = coord.x / regionSize;
        int regionY = coord.y / regionSize;
    ...
    public void Move (AgentInfo e, Coord fromCoord, Coord toCoord) {
        Remove (e, fromCoord);
        Add (e, toCoord);
    }
    ...
    public void DrawDebugGizmos (Coord coord, float viewDst) {
        ...
        float height = EcoManagmentSystem.tileCentres[0, 0].y + 0.1f;
